...
Projekt:
16.Znajdź i zaznacz obwód trójek na zdjęciu. Policz liczbę czarnych symboli i je zaznacz.
Zidentyfikuj karty 7 kier i 3 trefl oraz połącz ich środki.
...
void Mono(Image3CH& rgbImg, Image1CH& grayImg) //Przykład 
{
	using namespace std;
...
void Maksimum1CH(Image1CH& we, Image1CH& wy, int okno_rozmiar)
{
	...
	{
		int ramka = (okno_rozmiar - 1) / 2; //Grubość ramki w pikselach
		...
		for (int i = 0; i < we.width(); i++)
		{
			...
			{
				if (i >= ramka && i < we.width() - ramka && j >= ramka && j < we.height() - ramka) // Czy jest poza ramką
				{
					//Wpisanie do wektora okna części pikseli obrazu wejściowego
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Intensity() = okno[okno_rozmiar_kwadrat - 1]; //Wpisanie maksymalnej wartości intensywności z wektora
				}
				else
				{
					wy(i, j).Intensity() = we(i, j).Intensity();	//Filtracja ramki jest niewymagana przy danych zdjęciach
				}
			}
		}
		delete[] okno;
		...
		for (int i = 0; i < we.width(); i++)
		{
			for (int j = 0; j < we.height(); j++)
			{
				if (i >= ramka && i < we.width() - ramka && j >= ramka && j < we.height() - ramka) // Czy jest poza ramką
				{
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Red() = okno[okno_rozmiar_kwadrat - 1]; //Wpisanie maksymalnej wartości intensywności z wektora
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Green() = okno[okno_rozmiar_kwadrat - 1]; //Wpisanie maksymalnej wartości intensywności z wektora
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Blue() = okno[okno_rozmiar_kwadrat - 1]; //Wpisanie maksymalnej wartości intensywności z wektora

				}
				else
				{
					//Filtracja ramki jest niewymagana przy danych zdjęciach
		...
		for (int i = 0; i < we.width(); i++)
		{
			for (int j = 0; j < we.height(); j++)
			{
				if (i >= ramka && i < we.width() - ramka && j >= ramka && j < we.height() - ramka) // Czy jest poza ramką
				{
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Intensity() = okno[0]; //Wpisanie minimalnej wartości intensywności z wektora
				}
				else
				{
					wy(i, j).Intensity() = we(i, j).Intensity();	//Filtracja ramki jest niewymagana przy danych zdjęciach
		...
		for (int i = 0; i < we.width(); i++)
		{
			for (int j = 0; j < we.height(); j++)
			{
				if (i >= ramka && i < we.width() - ramka && j >= ramka && j < we.height() - ramka) // Czy jest poza ramką
				{
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Red() = okno[0]; //Wpisanie minimalnej wartości intensywności z wektora
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Green() = okno[0]; //Wpisanie minimalnej wartości intensywności z wektora
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Blue() = okno[0]; //Wpisanie minimalnej wartości intensywności z wektora
...
void Medianowy1CH(Image1CH& we, Image1CH& wy, int okno_rozmiar)
{
	...
	if (we.width() == wy.width() && we.height() == wy.height())
	{
		...
		int okno_rozmiar_kwadrat = okno_rozmiar * okno_rozmiar;
		double* okno = new double[okno_rozmiar_kwadrat]; //Wektor z pikseli okna
		int polowa_wektora_okna = (okno_rozmiar_kwadrat - 1) / 2; //Długość woktora zawsze nieparzysta
		...
		for (int i = 0; i < we.width(); i++)
		{
			for (int j = 0; j < we.height(); j++)
			{
				if (i >= ramka && i < we.width() - ramka && j >= ramka && j < we.height() - ramka) // Czy jest poza ramką
				{
					...
					sort(okno, okno + okno_rozmiar_kwadrat); //Sortowanie rosnąco
					wy(i, j).Blue() = okno[polowa_wektora_okna]; //Wpisanie minimalnej wartości intensywności z wektora
...
void BinaryzacjaSektorowa1CH(Image1CH& we, Image1CH& wy, int okno_rozmiar, double odciecie)
{
	...
	if (we.width() == wy.width() && we.height() == wy.height())
	{
		...
		for (int i = 0; i < we.width(); i++)
		{
			for (int j = 0; j < we.height(); j++)
			{
				//Sprawdzenie odcięcia - skrajne wartości - przyśpiesza całość
				...
				else if (i >= ramka && i < we.width() - ramka && j >= ramka && j < we.height() - ramka) // Czy jest poza ramką			
				{
					//Próg jakos wartość średnia z sektora/okna pikseli
					double suma = 0;
					for (int x = 0; x < okno_rozmiar; x++)
					{
						for (int y = 0; y < okno_rozmiar; y++)
...
void SegmentacjaKart1CH(Image1CH& we)  //Zwraca ilosc segmentow
{
	...
	//2krok - w jedną stronę   		//Z filtra
	int okno_rozmiar = 3;
	int ramka = (okno_rozmiar - 1) / 2; //Odległość od środka ramki
	int okno_rozmiar_kwadrat = okno_rozmiar * okno_rozmiar;
	...
	for (int i = 0; i < SZER; i++)
	{
		for (int j = 0; j < WYS; j++)
		{
			...
			{
				if (i >= ramka && i < SZER - ramka && j >= ramka && j < WYS - ramka) // Czy jest poza ramką
				{
					//Wpisanie do wektora okna części etykiet
					//Z filtra minimum...
					for (int x = 0; x < okno_rozmiar; x++)
					{
						for (int y = 0; y < okno_rozmiar; y++)
					...
					while (okno[index] == 0)
						index++;
					EtykietyKart[i][j] = okno[index]; //Wpisanie minimalnej wartości z wektora
				}
			}
		}
	}

	//2krok - w drugą stronę   	
...
int LiczEtykietyKart()
{
	...
	std::sort(etykiety, WYS * SZER + etykiety); //Sortowanie rosnąco
	...
	if (we.width() == wy.width() && we.height() == wy.height())
	{
		...
		for (int i = 0; i < we.width(); i++)
		{
			for (int j = 0; j < we.height(); j++)
			{
				if (we(i, j).Intensity() == 1)
				{
					wy(i, j).Intensity() = double(segmentId) / 1024 / 1024; // Może przesunięcia bitowe coś optymalizują
					segmentId++;
				}
				else
				{
		...
		for (int i = 0; i < wy.width(); i++)
		{
			for (int j = 0; j < wy.height(); j++)
			{
				...
				{
					if (i >= ramka && i < wy.width() - ramka && j >= ramka && j < wy.height() - ramka) // Czy jest poza ramką
					{
						//Z filtra minimum...
						//Wpisanie do wektora okna części pikseli obrazu wejściowego
						for (int x = 0; x < okno_rozmiar; x++)
						...
						while (okno[index] == 0)
							index++;
						wy(i, j).Intensity() = okno[index]; //Wpisanie minimalnej wartości intensywności z wektora
					}
					else
					{
						wy(i, j).Intensity() = 0; // Zbędne
					}
...
void MinimalizacjaEtykietKart()
{
	...
	sort(etykiety, WYS * SZER + etykiety); //Sortowanie rosnąco

	//Zapisanie wartości początkowych etykiet
	for (int i = 0; i < WYS * SZER - 1; i++)
	{
	...
	for (int i = 0; i < SZER; i++)
	{
		for (int j = 0; j < WYS; j++)
		{
			...
			{
				indeksEtykiety = 0;
				while (EtykietyKart[i][j] != etykietyZredukowane[indeksEtykiety]) // Musi być taka wartość
				{
					indeksEtykiety++;
				}
...
void EtykietyNa1CH(Image1CH& wy, double min, double max)
{
	...
	//Skalowanie wartości intensywnosci na zakres min - max
	for (int i = 0; i < wy.width(); i++)
	{
		for (int j = 0; j < wy.height(); j++)
		{
